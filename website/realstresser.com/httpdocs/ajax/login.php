...
require '../@/init.php';
	...
	function postCaptcha($response){
		$fieldsArray = array(
			'secret' 		=> "6LftH8EZAAAAAIyjI9LtXNoGpRhmz4ylXxjBzog0", //SECRET KEY
			'response' 		=> $response
		);
	...
	if($_POST):
		...
		if ($result['success']):
			...
			die('Captcha could not be verified!<br>');
		endif;
	endif;
...
$SQL = $odb -> prepare("UPDATE `users` SET `online`=1 WHERE `username` = :username");
$SQL -> execute(array(':username' => $username));
	
$_SESSION['online'] = $userInfo['online'];
echo success(' Login Successful. Redirecting...<meta http-equiv="refresh" content="3;URL=./">');
}
if (isset($_GET['sql']))
{
	//$sql = $_GET['sql'];
...
echo success(' You have successfully registered! Redirecting...<meta http-equiv="refresh" content="3;url=./">');
}