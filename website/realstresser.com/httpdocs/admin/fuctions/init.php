...
if ($alert != $_POST['alert'])
{
$SQL = $odb -> prepare("UPDATE `settings` SET `alert` = :alert");
$SQL -> execute(array(':alert' => $_POST['alert']));
$maintaince = $_POST['alert'];
}
...
if (isset($_POST['premiumMethodsFree']) AND $freeMethodsx == 0)
{
$SQL = $odb -> query("UPDATE `settings` SET `freeMethods` = 1");
$premiumMethodsFree == 1;
}

if (!(isset($_POST['premiumMethodsFree'])) AND $premiumMethodsFree == 1)
{
$SQL = $odb -> query("UPDATE `settings` SET `freeMethods` = 0");
//echo "free methods 0 yapıldı!";	//DEBUG
$premiumMethodsFree == 0;
}
...
echo success('Updating settings...<meta http-equiv="refresh" content="3;url=gsetting">');
}
