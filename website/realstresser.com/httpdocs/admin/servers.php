...
$error = 'Slots field has to be numeric';
	...
	if (empty($_POST['api']) || empty($_POST['name']) || empty($_POST['slots']) || empty($_POST['methods']) || empty($_POST['method']))
{
$error = 'Please verify all fields';
}
						else
$SQLinsert = $odb -> prepare("INSERT INTO `api` VALUES(NULL, :name, :api, :slots, :methods, :method)");
$SQLinsert -> execute(array(':api' => $api, ':name' => $name, ':slots' => $slots, ':methods' => $methods, ':method' => $method));
echo success('API has been added <meta http-equiv="refresh" content="3;url=servers">');
}
else
{
error($error);
				...
				?>
									...
									<div class="row-form">
                                        <div class="col-md-4"><strong>Method Type:</strong></div>
                                        <div class="col-md-12"><input type="text" class="form-control" name="method" placeholder="L7 or L4"/></div>
                                    </div>
                                    <div class="row-form">
...
while($getInfo = $SQLGetMethods -> fetch(PDO::FETCH_ASSOC))
{
 ...
 echo '<tr><td>'.htmlspecialchars($name).'</td><td>Encrypted</td><td>'.htmlspecialchars($slots).'</td><td>'.htmlspecialchars($method).'</td><td class="row">'.htmlspecialchars($methods).'</td><td><button type="submit" title="Delete API" name="apidrop" value="'.htmlspecialchars($id).'" class="btn btn-danger btn-icon"><i data-feather="delete"></i></button></td></tr>';
}
...
echo success('API has been removed <meta http-equiv="refresh" content="3;url=servers">');
}
?>
</form>								
                                    </table>
